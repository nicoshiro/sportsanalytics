...
print('Loading data...')
pbp_combined <- read_csv('../data/pbp/2015-16/[10-20-2015]-[06-20-2016]-combined-stats.csv')
# fix game_id column
...
for (current_game_id in game_ids){
  ...
  pbp_table <- pbp_table %>%
    ...
    mutate(minutes_played = 60 * as.numeric(substr(pbp_table$play_time,1,2)) +
             as.numeric(substr(pbp_table$play_time,4,5)) +
             1/60*as.numeric(substr(pbp_table$play_time,7,8))) %>%
    dplyr::select(-play_time)

  # join consecutive observations for each stint into one observation
  previous_players <- pbp_table[1,] %>% select(a1:h5) %>% unlist() %>%
    as.vector() %>% sort()
  for (row in 2:nrow(pbp_table)){
    current_players <- pbp_table[row,] %>% select(a1:h5) %>% unlist() %>%
      as.vector() %>% sort()
    if (setequal(current_players, previous_players)){
      # add minutes played to current row since stints are joined
      pbp_table[row,'minutes_played'] <- pbp_table[row-1,'minutes_played'] +
        pbp_table[row,'minutes_played']
      # ignore previous row
      pbp_table[row-1,] <- NA
    }
